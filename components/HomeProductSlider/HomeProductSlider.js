    ...
    return (
        <>
            <swiper-container ref={sliderRef} class="grid w-full fixed z-10" slides-per-view="1.9" centered-slides="true" loop>
                {data.map((item, index) => 
                    <swiper-slide class="flex items-center h-screen relative z-50" key={index}>
                        <Link className="w-[85%] lg:w-[80%] xl:w-[75%] 2xl:w-[70%] max-w-2xl mx-auto" href={`product/${item.attributes.urlHandle}`}>
            ...
            <div ref={controlsRef} className="hidden lg:block group" onMouseMove={updateCursorPosition}>
                <button aria-label="Prev slide" className="cursor-none z-10 fixed w-1/3 left-0 top-0 h-full rotate-180" onClick={() => sliderRef.current.swiper.slidePrev()}></button>
                <button aria-label="Next slide" className="cursor-none z-10 fixed w-1/3 right-0 top-0 h-full" onClick={() => sliderRef.current.swiper.slideNext()}></button>
