    ...
    return (
        <>
            ...
            <main className="main-reveal px-4 pb-4 flex flex-col gap-4 lg:flex-row">
                <div className="lg:max-w-9/12 xl:max-w-4/5 2xl:max-w-5/6">
                    <Image blurDataURL={`http://localhost:1337${currentProduct.data[0].attributes.image.data.attributes.url}`} placeholder="blur" unoptimized className="w-auto" priority src={`http://localhost:1337${currentProduct.data[0].attributes.image.data.attributes.url}`} width={2000} height={2000} alt=""/>
                </div>
                <div>
