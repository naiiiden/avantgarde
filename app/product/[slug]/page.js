            ...
            <HeaderHeightStickyVal className={"pb-4"}>
                <div className={`max-w-[18.75rem] ml-auto text-sm ${!currentProduct.attributes?.image?.data?.attributes?.url ? 'lg:pb-4' : ''}`}>
                    <div className="max-lg:px-4">
                        <h1 className="text-base"><span className="font-semibold">{currentProduct.attributes.name}</span> by {currentProduct.attributes.creator}, <span className="font-semibold">{currentProduct.attributes.date}</span></h1>
                        {currentProduct.attributes.description !== null && <p className="my-4">{currentProduct.attributes.description}</p>}
